...
\begin{*definition}
	...
	\begin{enumerate}
		...
		\item (a subset) $C \subseteq \Rn$ is \begriff{convex}, $x,y \in C$, the segements from $x,y$ is lies entirely in $C$.
		\item Let $A \subseteq \Rn$, \begriff{convex hull} of $A$ is the intersection of all sets in $\Rn$, which contains $A$.
		\item \begriff{$p$-simplex} $s$ is the convex hull of a collection of $(p+1)$ points $\set{x_0, \dots, x_p}$
	\end{enumerate}
\end{*definition}
...
\begin{*remark}
	...
	\begin{itemize}
		...
		\begin{align*}
			\sum s_i = 1
		\end{align*}
		this all could come from barycentric coordinates?!
		\item Should $p$ be not greater than $n$? At least when we compare to Croom's Basics Concepts of Algebraic Topology 7.4 Singular Homology. 
		\end{itemize}
	}
\end{*remark}
